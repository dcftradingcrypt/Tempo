...
\- pathUSD / AlphaUSD / BetaUSD / ThetaUSD を `SINK\_ADDRESS` に送付
...
&nbsp; `Permit2.approve(AlphaUSD, SINK\_ADDRESS, MaxUint160, MaxUint48)`



> faucetは禁止。`tempo\_fundAddress` 等は一切実装しない。
...
\- 復号は実行時に `WALLET\_PASSWORD` で行う（ログに秘密鍵/パスワードは出さない）
...
&nbsp; - 送った各txの: hash / explorer URL

&nbsp; - `eth\_getTransactionReceipt` の raw 取得結果（feeToken / feePayer を含める）
...
&nbsp;  └─ wallet.enc        (git tracked / passwordはGitHub Secrets)
